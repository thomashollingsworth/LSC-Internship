...
int main(void){
    ...
    std::vector<std::vector<double> > flux(num_xcells+4,std::vector<double>(3, 0.0));
    ...
    std::vector<std::vector<double> > u(num_xcells+4, std::vector<double>(3, 0.0));
    
    u=set_u0(x,gamma);
    
    std::vector<std::vector<double> > uPlus1(num_xcells+4, std::vector<double>(3, 0.0));
    ...
    do {
        ...
        for(size_t i=2; i<num_xcells+2; i++){
            for(size_t j=0;j<3;j++){
                uPlus1[i][j]=u[i][j] + (dt/dx) * (flux[i-1][j]-flux[i][j]);
