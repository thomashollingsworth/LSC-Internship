...
void MHD_xflux(StateVector& out, const ConservedStateVector& CSV, const PrimitiveStateVector& PSV,const double c_h){
    ...
    out[1]=CSV.density()*(PSV.velocity().x()*PSV.velocity().x()) + PSV.pressure() + 0.5*dot(CSV.B(),CSV.B())-(CSV.B().x()*CSV.B().x());
    ...
    out[4]=PSV.velocity().x()*(CSV.energy()+PSV.pressure()+0.5*dot(CSV.B(),CSV.B()))-dot(PSV.velocity(),CSV.B())*CSV.B().x();
    out[5]=CSV.psi();
...
void MHD_yflux(StateVector& out, const ConservedStateVector& CSV, const PrimitiveStateVector& PSV,const double c_h){
    ...
    out[4]=PSV.velocity().y()*(CSV.energy()+PSV.pressure()+0.5*dot(CSV.B(),CSV.B()))-dot(PSV.velocity(),CSV.B())*CSV.B().y();
